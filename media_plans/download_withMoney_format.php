...
$netCost = money_format('%!i', $netCost);
$gst = money_format('%!i', $gst);
$grossTotal = $netCost+$gst;
$grossTotal = money_format('%!i', $grossTotal);
...
for ($i=1; $i <= $quotient; $i++) { 
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('L'.$cellD, money_format('%!i', $mergeArray['Budget'.$i]));
